...
export const fetchArtist = (id) => dispatch => {
  return APIUtil.fetchArtist(id).then(
    (artist) => (dispatch({ type: RECEIVE_ARTIST, artist }), errors => dispatch({ type: RECEIVE_ERRORS, errors: errors.responseJSON })
    ));
};

export const fetchArtists = () => dispatch => {
  return APIUtil.fetchArtists().then(
    (artists) => (dispatch({ type: RECEIVE_ARTISTS, artists }), errors => dispatch({ type: RECEIVE_ERRORS, errors: errors.responseJSON })
    ));
};
